...
SELECT 
    ...
    1 - (f.embedding <=> array_fill(0.01, ARRAY[768])::vector) AS similitud_coseno
FROM fragmentos f
...
ORDER BY f.embedding <=> array_fill(0.01, ARRAY[768])::vector
LIMIT 5;
...
SELECT 
    f.fragmento_id,
    f.contenido_texto,
    1 - (f.embedding <=> array_fill(0.01, ARRAY[768])::vector) AS similitud
FROM fragmentos f
JOIN versiones_documento v ON f.version_id = v.version_id
JOIN documentos d ON v.documento_id = d.documento_id
WHERE v.es_activa = TRUE 
